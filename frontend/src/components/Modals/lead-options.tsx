      ...
      <div className="space-y-2">
       {getTemplates.data?.map(n => (
        <button
         key={n.position}
         type="button"
         className={`w-full flex items-center justify-center px-3 py-2 rounded border ${position === String(n.position) ? 'bg-primary text-white hover:bg-primary/90' : 'bg-background hover:bg-muted'}`}
         onClick={() => {
       ...
       {(getTemplates.data?.length ?? 0) < 5 && (
        <button
         key="new"
         type="button"
         className="w-full flex items-center justify-center px-3 py-2 rounded border bg-background hover:bg-muted"
         onClick={() => {
          // compute next available position 1..5
          const used = new Set(
           getTemplates.data?.map(t => Number(t.position)) || []
          )
          let next = 1
          for (let i = 1; i <= 5; i++) {
           if (!used.has(i)) {
