          ...
          <div className="w-40 pr-4 border-r">
            ...
            <div className="space-y-2">
              {getTemplates.data?.map(n => (
                <button key={n.position} type="button" className={`w-full h-10 flex items-center justify-center px-3 py-2 rounded border-2 ${n.active ? 'border-green-500' : 'border-border'} ${position === String(n.position) ? 'bg-primary text-white hover:bg-primary/90' : 'bg-background hover:bg-muted'}`}
                  onClick={() => {
              ...
              {(() => {
                ...
                const used = new Set((getTemplates.data?.map(t => Number(t.position)) || []).concat(extraPositions));

                return (
                  <>
                    {extraPositions.map(pos => (
                      <button key={`new-${pos}`} type="button" className={`w-full h-10 flex items-center justify-center px-3 py-2 rounded border bg-background text-muted`} onClick={() => { setPosition(String(pos)); setSubject(''); setBody(''); setActive(false); }}>
                        Novo template {pos}
                      </button>
                    ))}

                    {totalCount < 5 && (
                      <button key="new" type="button" className="w-full h-10 flex items-center justify-center px-3 py-2 rounded border bg-background hover:bg-muted"
                        onClick={() => {
