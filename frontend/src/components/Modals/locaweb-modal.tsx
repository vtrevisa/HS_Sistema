...
export function LocawebModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
	...
	const handleTest = async () => {
		...
		const testData = {
			host: form.host,
			port: form.port,
			encryption: form.encryption === '' ? null : form.encryption as 'tls' | 'ssl',
		...
		if (result.success) {
			...
			setFeedback({ type: 'error', message: result.error || 'Falha na conexão SMTP' });
		}
		setTesting(false);
	}
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		setSaving(true);
		setFeedback(null);
		console.log('Dados do formulário para salvar SMTP:', form, 'Provedor selecionado:', (form as any).provider);
		const submitData = {
			host: (form as any).provider === 'locaweb' ? 'email-ssl.com.br' : (form as any).provider === 'yahoo' ? 'imap.mail.yahoo.com' : form.host,
			port: (form as any).provider === 'locaweb' ? 465 : (form as any).provider === 'yahoo' ? 993 : form.port,
			encryption: (form as any).provider === 'locaweb' || (form as any).provider === 'yahoo' ? 'ssl' : form.encryption === '' ? null : form.encryption as 'tls' | 'ssl',
			username: form.username,
			password: form.password,
		...
		if (result.success) {
			...
			setFeedback({ type: 'error', message: result.error || 'Erro ao salvar configuração SMTP' });
		}
	}
			...
			<DialogContent className="max-w-lg">
		  	...
		  	<form onSubmit={handleSubmit} className="space-y-4">
		  		<div className="grid grid-cols-2 gap-4">
					...
					<div>
						<label className="block text-sm font-medium mb-1">E-mail de origem (From)</label>
						<input
							type="email"
							name="from_email"
					...
					<div>
						<label className="block text-sm font-medium mb-1">Nome de origem (opcional)</label>
						<input
							type="text"
							name="from_name"
				...
				<div className="flex items-center justify-between pt-2">
					<div className="flex items-center space-x-2">
						...
						/>
						<label htmlFor="advanced" className="text-sm text-muted-foreground">Configuração avançada</label>
					</div>
