...
export function ProfileAdminTable({ users }: ProfileAdminTableProps) {
 ...
 function handleToggleRole(userId: number, currentRole: string | undefined) {
  const newRole = currentRole === 'admin' ? 'user' : 'admin'
  console.log(`Alterado para ${newRole}`)
 }
        ...
        >
         ...
         <td className="p-4 align-middle [&:has([role=checkbox])]:pr-0 hidden md:table-cell">
          <div className="flex items-center gap-2">
           <span className="font-medium">{user.alvarasUsed}</span>
           <span className="text-muted-foreground">/</span>
           <span className="text-muted-foreground">
            {user.plan?.monthly_credits}
           </span>
          </div>
         </td>
         <td className="p-4 align-middle [&:has([role=checkbox])]:pr-0 hidden md:table-cell">
